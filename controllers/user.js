...
const login = async (req, res) => {
    ...
    const userEmail = await userModel.findOne({email:usernameoremail}).catch((error)=>res.status(400).send(error))
    const userUsername = await profileModel.findOne({username:usernameoremail}).catch((error)=>res.status(400).send(error))
    if(!userEmail && !userUsername) return res.status(400).send('Invalid credential')
    const goodPassowrd = await bcrypt.compare(password, userEmail.password).catch((error)=>res.status(400).send(error))
    if(goodPassowrd){
